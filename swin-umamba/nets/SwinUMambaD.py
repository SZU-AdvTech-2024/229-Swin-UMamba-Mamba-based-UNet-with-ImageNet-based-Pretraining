...
class VSSMEncoder(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, depths=[2, 2, 9, 2], 
        ...
        self.dims = dims
        self.bottleneck = BottleNeckBlock(768)
    ...
    def forward(self, x):
        ...
        # x_ret[4] = self.bottleneck(x_ret[4])
...
class SwinUMambaD(nn.Module):
    def __init__(self, vss_args, decoder_args,num_classes=1):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if isinstance(logits, list):
            if self.num_classes == 1:
                logits = list(F.interpolate(logit, size=(256,256), mode='bilinear') for logit in logits)
                return logits
        else:
            if self.num_classes == 1:
                return logits
