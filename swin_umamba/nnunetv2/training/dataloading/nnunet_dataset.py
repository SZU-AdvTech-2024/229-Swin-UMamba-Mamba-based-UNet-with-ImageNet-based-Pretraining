...
class nnUNetDataset(object):
    def __init__(self, folder: str, case_identifiers: List[str] = None,
        ...
        (containing auxiliary information). This is done for small evaluate so that we don't spend too much CPU time on
        reading pkl files on the fly during training. However, for large evaluate storing all the aux info (which also
