...
from datasets.dataset_thyroid import ImageFolder
from utils import DiceLoss


def trainer_synapse(args, model, snapshot_path):
    print("arg_list_dir:",args.list_dir)
    ...
    train_loader = DataLoader(db_train, batch_size=batch_size, shuffle=True, num_workers=args.num_workers,
                              pin_memory=True,
                              worker_init_fn=worker_init_fn)   #num_worker=0
...
def trainer_thyroid(args, model, snapshot_path):
    print("arg_list_dir:",args.list_dir)
    logging.basicConfig(filename=snapshot_path + "/log.txt", level=logging.INFO,
                        format='[%(asctime)s.%(msecs)03d] %(message)s', datefmt='%H:%M:%S')
    logging.getLogger().addHandler(logging.StreamHandler(sys.stdout))
    ...
    train_loader = DataLoader(db_train, batch_size=batch_size, shuffle=True, num_workers=0,
                              pin_memory=True,
                              worker_init_fn=worker_init_fn)   #num_worker=0
    val_loader = DataLoader(db_train, batch_size=batch_size, shuffle=False, num_workers=0,
                            pin_memory=True,
                            worker_init_fn=worker_init_fn)
    if args.n_gpu > 1:
        model = nn.DataParallel(model)
    ...
    ce_loss = CrossEntropyLoss()
    dice_loss = DiceLoss(num_classes)
    # loss_fn = dice_loss*0.5+bce_loss*0.5
    optimizer = optim.SGD(model.parameters(), lr=base_lr, momentum=0.9, weight_decay=0.0001)
    ...
    for epoch_num in iterator:
        ...
        # for image_batch, sampled_batch in tqdm(enumerate(train_loader), desc=f"Train: {epoch_num}", total=len(train_loader),
        #                                    leave=False):
            # image_batch, label_batch = sampled_batch['image'], sampled_batch['label']
            image_batch, label_batch = image_batch.cuda(), label_batch.cuda()
            ...
            # logging.info('Train: iteration : %d/%d, lr : %f, loss : %f' % (
            #     iter_num, epoch_num, lr_, loss.item()))
            batch_dice_loss += loss_dice.item()
            batch_ce_loss += loss_ce.item()
            if iter_num % 20 == 0:
        ...
        batch_dice_loss /= len(train_loader)
        batch_loss = 0.4 * batch_ce_loss + 0.6 * batch_dice_loss
        logging.info(' The Train epoch: %d : loss : %f, loss_ce: %f, loss_dice: %f' % (
            epoch_num, batch_loss, batch_ce_loss, batch_dice_loss))
        ...
        if (epoch_num + 1) % args.eval_interval == 0:
            ...
            with torch.no_grad():
                ...
                batch_dice_loss /= len(val_loader)
                batch_loss = 0.4 * batch_ce_loss + 0.6 * batch_dice_loss
                logging.info(' The Val epoch: %d : loss : %f, loss_ce: %f, loss_dice: %f' % (
                    epoch_num, batch_loss, batch_ce_loss, batch_dice_loss))
                if batch_loss < best_loss:
                    save_mode_path = os.path.join(snapshot_path, 'best_model.pth')
                    torch.save(model.state_dict(), save_mode_path)
